...
import { SparklesIcon, ZapIcon, CheckIcon } from "@/components/marketing/icons";
    ...
    <div className="py-16 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24 bg-white text-zinc-900">
      ...
      <div className="text-center max-w-4xl mx-auto space-y-6">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#d86f82]/10 border border-[#d86f82]/20 text-[#a8455a] text-xs font-mono font-semibold">
          <SparklesIcon className="w-3.5 h-3.5 text-[#d86f82]" /> Multi-Format Static & Copy Synthesizer
        </div>
        <h1 className="text-4xl sm:text-6xl font-display font-bold text-zinc-950 tracking-tight leading-[1.08]">
          Generate On-Brand Ads from your <span className="bg-gradient-to-r from-[#d86f82] via-[#c85c6f] to-[#a8455a] bg-clip-text text-transparent">Brand Kit</span>
        </h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {PRESET_TEMPLATES.map((tmpl) => (
              ...
              >
                ...
                <span className="text-[10px] text-muted-foreground leading-tight block">{tmpl.desc}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex justify-end">
          ...
          >
            {generating ? (
              ...
              <>
                <span>Generate High-ROAS Hooks</span> <SparklesIcon className="w-3.5 h-3.5" />
              </>
            )}
          </button>
        </div>
        ...
        <div className="pt-6 border-t border-black/[0.06] space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {hooks.map((hook, i) => (
              ...
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-zinc-900">{hook.title}</span>
                  ...
                  <div className={`p-4 rounded-xl bg-gradient-to-br ${hook.bgStyle} border border-black/[0.06] space-y-2`}>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-black/[0.04] text-[10px]">
                  <span className="text-[#a8455a] font-mono font-medium">{hook.tag}</span>
                  <span className="text-muted-foreground font-mono">1080 × 1080 Static</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="p-4 rounded-xl bg-zinc-50 border border-black/[0.06] flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-xs text-zinc-700">
              Active Selection: <strong className="text-zinc-950">{hooks[selectedHook]?.title}</strong> staged with color <code className="text-[#a8455a] font-bold">{brandColor}</code>
            </div>
