...
const PERSONAS: PersonaConfig[] = [
  {
    ...
    badge: 'Full Loop',
    tagline:
      'End-to-end autonomous discovery, creative remixing, and draft deployment',
    placeholder:
      'Run full growth loop, audit performance, or command the agent fleet…',
  },
  {
    id: 'spy',
  ...
  },
  {
    ...
    badge: 'Integrity',
    tagline:
      'Audits ad-network over-claiming against actual store orders and blended MER',
...
const QUICK_ACTIONS: QuickAction[] = [
  {
    ...
    icon: '⚡',
    prompt:
      'Reconcile platform spend against Shopify revenue and check tracking integrity.',
  ...
  },
  {
    ...
    icon: '🔍',
    prompt:
      'Discover top winning ads for competitor domains with longevity > 90 days.',
    actionType: 'spy',
  },
  {
    id: 'creative',
    ...
    icon: '🎨',
    prompt:
      'Generate high-converting creative hook angles and script variants for our campaign.',
    actionType: 'creative',
  },
  {
    id: 'loop',
    ...
    icon: '🚀',
    prompt:
      'Run full autonomous growth loop: Find spy ads -> Score winners -> Remix creative -> Launch drafts.',
    actionType: 'loop',
  },
  {
    id: 'iroas',
    ...
    icon: '📊',
    prompt:
      'Evaluate true incremental ROAS across Meta, Google, and TikTok channels.',
    actionType: 'iroas',
  },
  {
    id: 'agents',
...
const PLATFORM_TOOLS = [
  {
    ...
    label: 'Google Ads',
    description:
      'Pull active campaigns with spend, impressions, clicks and conversions.',
  },
  {
    name: 'meta_ads.fetch_adsets',
    label: 'Meta Ads',
    description:
      'Pull ad sets with spend, impressions, CTR, and conversion metrics.',
  },
  {
    name: 'shopify.fetch_orders',
    label: 'Shopify',
    description:
      'Fetch recent store orders as the source of truth for actual revenue.',
  },
  {
    name: 'slack.send_message',
    label: 'Slack Alert',
    description:
      'Send live performance briefing or anomaly alert to #growth channel.',
  },
  {
    name: 'linear.create_issue',
    label: 'Linear Ticket',
    description:
      'Create a follow-up action ticket on the growth engineering board.',
  },
  {
    name: 'github.create_pr',
    label: 'GitHub Draft',
    description:
      'Open a pull request to apply approved ad copy or creative changes.',
  },
];
...
export default function CommandCenterPage() {
  ...
  const [activeView, setActiveView] = useState<'chat' | 'tools' | 'telemetry'>(
    'chat',
  );
  ...
  // Live telemetry data
  const [pipelineState, setPipelineState] = useState<PipelineResult | null>(
    null,
  );
  const [reconcileState, setReconcileState] = useState<ReconcileResult | null>(
    null,
  );
  ...
  const [messages, setMessages] = useState<CommandMessage[]>([
    {
      ...
      timestamp: fmtTime(),
      content:
        '**PerfOS Central Command Center** initialized. I am your end-to-end performance marketing operator.\n\nAsk me in plain language to audit over-claiming spend, spy on competitor ad longevity, generate high-converting creative hook variations, or run the full autonomous growth loop. All spend changes remain safety-gated for your approval.',
    ...
    async (text: string) => {
      ...
      try {
        ...
        if (
          ...
          const statusText =
            overPct > 15
              ? `⚠️ **Tracking Integrity Warning:** Ad platforms are over-claiming revenue by **${fmtPct(overPct)}** ($${recon.over_count_value.toLocaleString()}) compared to actual Shopify transactions.`
        ...
        // 2. Competitor Spy / Ad Discovery Intent
          ...
          const queryTerm =
            lower
              .replace(/spy|competitor|ads|for|find|search|evergreen/g, '')
              .trim() || 'saas';
          const results = await searchAdLibrary({
            query: queryTerm,
            limit: 6,
          }).catch(async () => {
        ...
        // 3. Creative Generation / Hook Remixing Intent
          ...
          const res = await generateCreative(
            activePersona === 'creative' ? 'saas' : 'dtc',
          );
          const agentMsg: CommandMessage = {
            id: `m-${Date.now() + 1}`,
            role: 'agent',
        ...
        // 5. iROAS / Incrementality Intent
          ...
          lower.includes('iroas') ||
          lower.includes('incrementality') ||
          lower.includes('lift')
        ) {
        ...
        const errorMsg: CommandMessage = {
          ...
          timestamp: fmtTime(),
          content:
            'Encountered an issue reaching the execution pipeline. Running with verified local cache.',
        };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Autonomous performance marketing orchestration · Reconciled
              against Shopify orders
            </p>
          </div>
        </div>
            ...
            {messages.map((m) => (
              ...
              >
                ...
                >
                    ...
                    m.dataPayload && (
                      <div className="mt-4 rounded-lg border border-border bg-card p-3 space-y-3">
                        <div className="flex items-center justify-between border-b border-border pb-2">
                          ...
                          <Badge
                            variant={
                              m.dataPayload.over_count_pct > 15
                                ? 'destructive'
                                : 'success'
                            }
                          >
                        ...
                        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                          <Stat
                            ...
                            value={fmtMoney(m.dataPayload.total_spend)}
                          />
                          <Stat
                            label="Claimed Revenue"
                            value={fmtMoney(
                              m.dataPayload.platform_claimed_value,
                            )}
                          />
                          <Stat
                            label="Shopify Revenue"
                            value={fmtMoney(m.dataPayload.actual_revenue)}
                          />
                          <Stat
                            label="Over-Count %"
                            value={fmtPct(m.dataPayload.over_count_pct)}
                            sub={`MER ${fmtMer(m.dataPayload.blended_mer)}`}
                          />
                        </div>
                        <div className="flex justify-end gap-2 pt-1 border-t border-border">
                          <Button
                            ...
                            onClick={() =>
                              void processCommand(
                                'Generate recommendations based on this reconciliation',
                              )
                            }
                          >
                  ...
                  {m.role === 'agent' &&
                    m.interactiveType === 'discovery' &&
                    Array.isArray(m.dataPayload) && (
                      <div className="mt-4 space-y-2">
                        ...
                        <div className="grid gap-2 sm:grid-cols-2">
                          {m.dataPayload
                            .slice(0, 4)
                            .map((ad: AdLibraryItem, idx: number) => (
                              <div
                                key={ad.ad_id || idx}
                              ...
                              >
                                ...
                                <p className="text-[11px] text-muted-foreground line-clamp-2">
                                  {ad.title ||
                                    ad.body ||
                                    'Consolidate your workflow into a unified system.'}
                                </p>
                                <div className="pt-2 flex justify-between items-center border-t border-border/60">
                                  ...
                                  <Button
                                    ...
                                    onClick={() =>
                                      void processCommand(
                                        `Remix hook for ${ad.advertiser || 'this competitor ad'}`,
                                      )
                                    }
                                  >
                                    Remix Hook →
                    ...
                    <div className="mt-4 rounded-lg border border-border bg-card p-3 space-y-2">
                      ...
                      <div className="space-y-1.5 text-xs text-zinc-300">
                        <div className="p-2 rounded bg-muted/60 border border-border/60">
                          ...
                          <span>
                            "Why are 84% of performance marketers bleeding 30%
                            of their ad budget on fake platform attribution?"
                          </span>
                        </div>
                        <div className="p-2 rounded bg-muted/60 border border-border/60">
                          <span className="text-primary font-semibold">
                            Angle #2 (Founder Voiceover):{' '}
                          </span>
                          <span>
                            "We audited $2.4M in DTC ad spend. Here are the 3
                            settings that Meta hides from you."
                          </span>
                        </div>
                      </div>
                      <div className="pt-2 flex justify-end gap-2">
                        <Link href="/creative">
                          <Button
                            size="sm"
                            className="btn-daisy-solid h-7 text-xs"
                          >
                            ...
                            >
                              ...
                              <div className="flex items-center gap-4">
                                ...
                                <Badge
                                  ...
                                  className="text-[10px]"
                                >
                                  {Math.round(row.calibration * 100)}%
                                  Calibrated
                                </Badge>
                              </div>
                            </div>
            ...
            {isExecuting && (
              <div className="flex justify-start">
                <div className="flex items-center gap-2 rounded-xl border border-border bg-surface px-4 py-3 text-xs text-muted-foreground">
                  ...
                  <span>
                    Agent is executing command across performance stack…
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="border-t border-border bg-card p-3 sm:p-4">
            <form onSubmit={handleSubmit} className="relative">
              ...
              <div className="mt-1.5 flex items-center justify-between px-1 text-[10px] text-muted-foreground">
                <span>
                  Press <strong>Enter</strong> to dispatch ·{' '}
                  <strong>Shift+Enter</strong> for new line
                </span>
                <span className="font-mono">
                  Risk Policy: Human Approval Gate Active
                </span>
              </div>
            </form>
          </div>
            ...
            <Card className="bg-surface border-border">
              ...
              <CardContent>
                <p className="text-[11px] text-muted-foreground">
                  Across 6 connected ad channels
                </p>
              </CardContent>
            </Card>
            ...
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription className="text-xs">
                  Proposals waiting for human operator sign-off before API
                  dispatch.
                </CardDescription>
              </div>
              <Button
            ...
            <CardContent>
              {recommendations.length === 0 ? (
                <div className="rounded-lg border border-dashed border-border py-8 text-center text-xs text-muted-foreground">
                  No recommendations pending approval. The growth loop is
                  operating cleanly.
                </div>
              ) : (
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            variant={
                              rec.risk === 'high' ? 'destructive' : 'warning'
                            }
                          >
                            {rec.risk} risk
                          </Badge>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto pr-1">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Trigger connected MCP integration endpoints directly with
                payload tracking.
              </p>
            </div>
            <Button
