    ...
    <div className="space-y-4 py-1" aria-hidden="true">
      <div className="grid grid-cols-2 gap-4 pb-2 sm:grid-cols-4">
        {[0, 1, 2, 3].map((i) => (
          <div
            key={i}
            className="space-y-1.5 rounded-lg border border-white/4 bg-white/1.5 p-3"
          >
      ...
      {[0, 1, 2, 3, 4].map((i) => (
        <div
          key={i}
          className="space-y-2 rounded-lg border border-white/4 bg-white/1.5 p-3.5"
        >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="mt-1 max-w-sm text-xs leading-relaxed text-muted-foreground">
        Run incrementality tests or connect conversion webhooks to calculate
...
export function IroasChart({
  ...
  const maxIroas = React.useMemo(() => {
    ...
    const peak = Math.max(
      ...safeData.map((d) => {
        const iroasVal =
          typeof d.iroas === 'number' && !isNaN(d.iroas) ? d.iroas : 0;
        const reportedVal =
          typeof d.reported_roas === 'number' && !isNaN(d.reported_roas)
            ? d.reported_roas
            : iroasVal;
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="space-y-2.5 pt-1">
              {sorted.map((item) => {
                const iroasVal =
                  typeof item.iroas === 'number' && !isNaN(item.iroas)
                    ? item.iroas
                    : 0;
                const reportedVal =
                  typeof item.reported_roas === 'number' &&
                  !isNaN(item.reported_roas)
                    ? item.reported_roas
                    : null;

                const iroasPct = Math.min(
                  Math.max((iroasVal / maxIroas) * 100, 4),
                  100,
                );
                const reportedPct =
                  reportedVal !== null
