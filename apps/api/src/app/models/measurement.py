...
class CreativePerformance(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("hook_rate >= 0 AND hook_rate <= 1", name="ck_creative_hook_rate_range"),
    )
...
class IncrementalityTest(Base):
    ...
    markets_treated: Mapped[list[Any] | None] = mapped_column(JSON, default=None)
    markets_control: Mapped[list[Any] | None] = mapped_column(JSON, default=None)
    ...
    lift_pct: Mapped[float | None] = mapped_column(Float, default=None)
...
def compute_lift_pct(
    ...
    if not conversions_control or not spend_control or not spend_treated:
        return None
    return round(
        ((conversions_treated / conversions_control) / (spend_treated / spend_control) - 1) * 100,
        2,
    )
